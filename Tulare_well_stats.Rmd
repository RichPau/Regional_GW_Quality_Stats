---
title: "Regional Groundwater Quality Trends in California's Tulare Basin"
author: "Rich Pauloo"
date: "August 22, 2017"
output: 
  html_document:
...
center_coords <- tulare %>% 
  group_by(subregion) %>% 
  dplyr::summarise(lat = median(lat), long = median(long)) %>% 
  mutate(county = c('Fresno', 'Kern', 'Kings', 'Tulare'))
...
# load Bulletin 118 GW basin shapfiles
library(rgdal)
library(maptools)
...
tulare_alluvial <- s %>% 
  ...
  arrange(Basin_Subb)
tulare_alluvial <- tulare_alluvial[8:14, ]

# string of subbasin names for subsetting
...
# pull out coords of tulare_alluvial for annotations
tulare_coords <- tulare_alluvial@polygons[[2]]@Polygons[[1]]@coords
